...
class SupervisorAgent(Agent):
    ...
    def _execute_tdd_cycle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_commit_msg = self._generate_commit_message(
                tester, "test", self.cycle_count, test_result
            )
            self._commit_agent_changes("test", test_commit_msg)

            # 2. Implementer Agent: Make the test pass
            implementer = ImplementerAgent(
                str(self.work_dir), self.kata_description, self.config
            )
            ...
            impl_commit_msg = self._generate_commit_message(
                implementer, "feat", self.cycle_count, impl_result
            )
            ...
            refactor_commit_msg = self._generate_commit_message(
                refactorer, "refactor", self.cycle_count, refactor_result
            )
    ...
    def _generate_commit_message(
        self, agent, agent_type: str, cycle_count: int, agent_result: dict
    ) -> str:
        ...
        try:
            ...
            prompt = create_commit_message_prompt(
                agent_type, self.kata_content, agent_result, cycle_count
            )

            # Generate the commit message using the LLM
            commit_message = llm_provider.generate_text(prompt).strip()

            # Remove markdown code blocks if present (some LLMs wrap responses in ```)
            if commit_message.startswith("```"):
                # Remove opening code block marker and any language identifier
                lines = commit_message.split("\n")
                # First line is ``` or ```language - skip it
                if len(lines) > 1:
                    commit_message = "\n".join(lines[1:])
                else:
                    commit_message = commit_message.lstrip("`").strip()
            ...
            commit_message = (
                commit_message.replace("**", "").replace("*", "").strip()
            ...
            print(
                f"{self.name}: Failed to generate LLM commit message: {str(e)}, using default"
            )
