...
class TDDWorkflow:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        while self.cycle_count < self.max_cycles:
            ...
            if self.verbose:
                ...
                print(
                    f"{status_icon} Agent result: {'Success' if success else 'Failure'}"
                )
                if not success:
                    print(
                        f"⚠️  Failure reason: {result.get('failure_reason', 'Unknown')}"
                    )
    ...
    def _run_tester_agent(self) -> tuple[bool, Dict[str, Any]]:
        ...
        if test_result["success"]:
            ...
            if self.verbose:
                print(
                    "❌ Tester Agent: Test already passes - not a proper failing test"
                )
            return False, {
            ...
            if self.verbose:
                print(
                    "✅ Tester Agent: Test correctly fails - ready for implementation"
                )
            if self.config.enable_git_staging:
                self._stage_file(test_file_path)

            return True, {"test_file": test_file_path, "test_result": test_result}
    ...
    def _run_implementer_agent(self) -> tuple[bool, Dict[str, Any]]:
        ...
        implementation_file_path = self.implementer.save_implementation(
            implementation_data
        )

        if self.verbose:
            print(
                f"🔨 Implementer Agent: Wrote implementation to {implementation_file_path}"
            )
        ...
        if test_result["success"]:
            ...
            if self.config.enable_git_staging:
                ...
                self._commit_changes(
                    f"Implement solution for test in {os.path.basename(test_file)}"
                )
    ...
    def _run_refactorer_agent(self) -> tuple[bool, Dict[str, Any]]:
        ...
        if not test_result["success"]:
            if self.verbose:
                print(
                    "❌ Refactorer Agent: Tests failing before refactoring - cannot proceed"
                )
            return False, {
        ...
        # Refactor the code
        refactor_data = self.refactorer.refactor_code(
            self.work_dir, "Improve code quality after implementation"
        )
    ...
    def _initialize_git_repo(self) -> None:
        ...
        try:
            ...
            try:
                ...
                subprocess.run(
                    ["git", "add", "."], cwd=self.work_dir, capture_output=True
                )
                subprocess.run(
    ...
    def _stage_file(self, file_path: str) -> None:
        ...
        try:
            subprocess.run(
                ["git", "add", file_path], cwd=self.work_dir, capture_output=True
            )
        except Exception:
    ...
    def _commit_changes(self, message: str) -> None:
        ...
        try:
            subprocess.run(
                ["git", "commit", "-m", message], cwd=self.work_dir, capture_output=True
            )
        except Exception:
